package com.rime.portfolioproject.domain.entity

import com.rime.portfolioproject.domain.BaseEntity
...
class HttpInterface(httpServletRequest: HttpServletRequest) : BaseEntity(){
    ...
    var cookies: String? = httpServletRequest.cookies?.map { "${it.name}:${it.value}" }
                                                            ?.toString()
